...
public class GraphADT<T> implements IGraphADT<T> {
  ...
  protected class Path implements Comparable<Path> {
    ...
    public int compareTo(Path other) {
      ...
      if (cmp != 0)
        return (int)cmp; // use path distance as the natural ordering
  ...
  protected Path dijkstrasShortestPath(T start, T end) {
    ...
    if (vertices.get(start) == null || vertices.get(end) == null) {
      throw new NoSuchElementException("Path cannot be found");
    }
    PriorityQueue<Path> pq = new PriorityQueue<Path>();
    Vertex starting = vertices.get(start);
    Vertex ending = vertices.get(end);
    ArrayList<Edge> noted = new ArrayList<Edge>();
    Path ret = new Path(starting);
    for (Edge i : starting.edgesLeaving) {
      pq.add(new Path(ret, i));
    }
    while (!pq.isEmpty()) {
      Path path = pq.poll();
      if (path.end.equals(ending)) {
        return path;
      } else {
        for (Edge i : path.end.edgesLeaving) {
          if (!noted.contains(i)) {
            pq.add(new Path(path, i));
            noted.add(i);
          }
        }
      }
